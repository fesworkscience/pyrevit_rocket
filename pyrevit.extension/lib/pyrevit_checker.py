...
Проверяет Python код на совместимость с pyRevit (CPython 3)
...
Проверки:
    - Python 3 синтаксис (ast.parse)
    - #! python3 shebang в script.py файлах
...
class PyRevitChecker:
    """Проверка совместимости с pyRevit (CPython 3)."""
    ...
    def check_file(self, filepath):
        ...
        # Проверки
        self.check_python3_syntax(content)
        self.check_python3_shebang(lines)
    ...
    def check_python3_syntax(self, content):
        """Проверить синтаксис Python 3."""
        try:
            ast.parse(content)
        except SyntaxError as e:
            self.errors.append(
                "Синтаксическая ошибка Python 3: строка {}, {}: {}".format(
                    e.lineno, e.offset, e.msg
                )
            )

    def check_python3_shebang(self, lines):
        """Проверить наличие #! python3 shebang в script.py."""
        ...
        if '.pushbutton' not in self.current_file:
            return

        # Проверяем первую строку
        if lines and not lines[0].strip().startswith('#! python3'):
            self.errors.append(
                "Отсутствует shebang '#! python3' в первой строке script.py"
            )

    def check_application_run(self, lines):
        """Проверить Application.Run() (использовать ShowDialog())."""
        # Пропускаем сам чекер
        filename = os.path.basename(self.current_file) if self.current_file else ""
        if filename == 'pyrevit_checker.py':
            return

        for i, line in enumerate(lines, 1):
            code_part = line.split('#')[0]
            # Пропускаем строки со строковыми литералами (могут содержать текст об ошибке)
